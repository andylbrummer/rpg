<script lang="ts">
  ...
  function routeColor(danger: number, status: OverworldRoute['status']): string {
  ...
  function routeDashArray(status: OverworldRoute['status']): string {
  ...
  function handleRouteEnter(
    e: MouseEvent,
    r: { distance: number; dangerRating: number; terrain: string; status: OverworldRoute['status'] }
  ) {
